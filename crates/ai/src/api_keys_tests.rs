...
#[test]
fn custom_endpoint_url_allows_local_http_hosts() {
    for valid in [
        "https://api.example.com/v1",
        "https://openrouter.ai/api/v1",
        "https://8.8.8.8/v1",
        // Local fork: local/private hosts and plain HTTP are allowed so
        // self-hosted inference servers (Ollama, LM Studio, llama.cpp, vLLM)
        // can be registered.
...
fn endpoint_definitions_join_keys_fail_closed_and_recover() {
    warpui_core::App::test((), |mut app| async move {
        ...
        manager.read(&app, |manager, _| {
            ...
            assert_eq!(manager.custom_endpoints()[0].api_key, "");
            // Local fork: keyless endpoints are still shipped as providers.
            assert!(manager.custom_model_providers_for_request(true).is_some());
        });
    });
}
...
#[test]
fn has_any_key_true_for_endpoint_with_empty_api_key() {
...
#[test]
fn has_any_key_false_for_endpoint_with_empty_url_and_key() {
    let keys = ApiKeys {
        custom_endpoints: vec![endpoint("ep", "", "", &[("m", None)])],
        ..Default::default()
    };
    assert!(!keys.has_any_key());
}
...
#[test]
fn keyless_endpoints_are_included() {
    // Local fork: endpoints without an API key (e.g. Ollama) still ship to the
    // server as custom model providers.
    let mgr = make_manager(ApiKeys {
        custom_endpoints: vec![

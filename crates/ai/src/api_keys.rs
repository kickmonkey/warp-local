...
/// Local fork: allow `http` and local/private hosts so self-hosted inference
/// servers (Ollama, LM Studio, llama.cpp, vLLM on localhost or a LAN) can be
/// registered as custom endpoints. Only structural URL validity is enforced.
...
impl ApiKeys {
    pub fn has_any_key(&self) -> bool {
        self.openai.is_some()
            ...
            // Local fork: a configured endpoint counts even without an API key,
            // since local inference servers commonly require none.
            || self
                .custom_endpoints
                .iter()
...
impl ApiKeyManager {
    ...
    /// provider or custom-endpoint key, a configured custom endpoint (local
    /// fork: keyless endpoints count — local inference servers need no key),
    /// or a connected Grok subscription.
    pub fn has_any_key(&self) -> bool {
    ...
    ///
    /// Returns `None` when custom models should not be included or no endpoint has a
    /// non-empty URL. Local fork: an API key is optional, since local inference
    /// servers (e.g. Ollama) commonly require none.

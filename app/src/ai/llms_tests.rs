...
#[test]
fn custom_llm_infos_include_keyless_endpoints() {
    // Local fork: endpoints without an API key (e.g. Ollama) surface in the
    // picker too.
    let keys = ai::api_keys::ApiKeys {
        custom_endpoints: vec![
            endpoint("keyless", "https://a.io", "", vec![model("m", None, "uuid-x")]),
            endpoint(
                "good",
                "https://b.io",
                "k",
    ...
    let infos = build_custom_llm_infos(&keys.custom_endpoints);
    assert_eq!(infos.len(), 2);
    assert!(infos.iter().any(|i| i.id.as_str() == "uuid-x"));
    assert!(infos.iter().any(|i| i.id.as_str() == "uuid-y"));
}

...
///
/// Endpoints with an empty URL, and models with empty name or config_key, are
/// skipped — they shouldn't surface in the picker until the user finishes configuring them.
/// Local fork: an API key is optional, since local inference servers (e.g. Ollama)
/// commonly require none.
fn build_custom_llm_infos(endpoints: &[CustomEndpoint]) -> Vec<LLMInfo> {
    endpoints
        .iter()
        .filter(|ep| !ep.url.trim().is_empty())
        .flat_map(|endpoint| {
            endpoint
                .models
                .iter()

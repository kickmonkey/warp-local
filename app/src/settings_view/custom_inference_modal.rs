...
impl CustomEndpointModal {
    pub fn new(
        ...
        let endpoint_url_editor = ctx.add_typed_action_view(move |ctx| {
            ...
            let mut editor = EditorView::single_line(options, ctx);
            editor.set_placeholder_text("e.g., http://localhost:11434/v1", ctx);
            if let Some(ep) = endpoint {
        ...
        let api_key_editor = ctx.add_typed_action_view(move |ctx| {
            ...
            let mut editor = EditorView::single_line(options, ctx);
            editor.set_placeholder_text("Optional (empty for local servers)", ctx);
            if let Some(ep) = endpoint {
...
fn is_endpoint_form_valid(name: &str, url: &str, _api_key: &str, has_models: bool) -> bool {
    // Local fork: the API key is optional so keyless local inference servers
    // (e.g. Ollama) can be registered.

...
fn validate_url_allows_localhost_and_private_ips() {
    // Local fork: local/private hosts are allowed for self-hosted inference.
    assert_eq!(validate_url("https://localhost:8080"), Ok(()));
    assert_eq!(validate_url("http://localhost:11434/v1"), Ok(()));
    assert_eq!(validate_url("http://127.0.0.1/v1"), Ok(()));
    assert_eq!(validate_url("https://0.0.0.0/v1"), Ok(()));
    assert_eq!(validate_url("https://10.0.0.1/v1"), Ok(()));
    assert_eq!(validate_url("https://172.16.0.1/v1"), Ok(()));
    assert_eq!(validate_url("https://192.168.0.1/v1"), Ok(()));
    assert_eq!(validate_url("https://169.254.0.1/v1"), Ok(()));
    assert_eq!(validate_url("https://[::1]/v1"), Ok(()));
    assert_eq!(validate_url("https://[::]/v1"), Ok(()));
    assert_eq!(validate_url("https://[fc00::1]/v1"), Ok(()));
    assert_eq!(validate_url("https://[fe80::1]/v1"), Ok(()));
    assert_eq!(validate_url("https://[::ffff:192.168.0.1]/v1"), Ok(()));
}

#[test]
fn endpoint_form_valid_accepts_http_local_url() {
...
#[test]
fn endpoint_form_valid_allows_empty_api_key() {
    // Local fork: keyless local inference servers are valid.
    assert!(is_endpoint_form_valid(
        "Ollama",
        "http://localhost:11434/v1",
        "",
...
#[test]
fn endpoint_form_valid_rejects_non_http_scheme() {
    assert!(!is_endpoint_form_valid(
        "Endpoint",
